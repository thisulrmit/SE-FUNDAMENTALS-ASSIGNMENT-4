...
public class Post {
    ...
    private boolean isValidTags(String[] tags, String type) {
        if (tags.length < 2 || tags.length > 5) {
            System.out.println("Debug: Invalid number of tags");
            return false;
        }
        if (type.equals("Easy") && tags.length > 3) {
            System.out.println("Debug: Easy post has more than 3 tags");
            return false;
        }

        for (String tag : tags) {
            if (tag.length() < 2 || tag.length() > 10 || !tag.equals(tag.toLowerCase())) {
                System.out.println("Debug: Invalid tag: " + tag);
                return false;
            }
        }
        return true;
    ...
    private boolean isValidEmergency(String emergency, String type) {
        if (type.equals("Very Difficult") || type.equals("Difficult")) {
            if (emergency.equals("Ordinary")) {
                System.out.println("Debug: Difficult post has Ordinary status");
                return false;
            }
        } else if (type.equals("Easy")) {
            if (emergency.equals("Immediately Needed") || emergency.equals("Highly Needed")) {
                System.out.println("Debug: Easy post has Immediately Needed or Highly Needed status");
                return false;
            }
        }
        return true;
